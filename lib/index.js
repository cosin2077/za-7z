...
const zipExecStr = (name) => `7z a "${name}.za.zip" "${name}" -p${password} -mx0`;

const checkCommand = (command) => {
  try {
    child.execSync(`command -v ${command}`);
...
function checkDestDir() {
  destDir = argv._[0];
  if (!destDir) {
    console.log(colors.red(`Error: destDir needed!`));
    process.exit(1);
  }
  destPath = path.resolve(process.cwd(), destDir);
  if (!fs.existsSync(destPath)) {
    console.log(colors.red(`Error: dir or filr ${destDir} not exists!`));
    process.exit(1);
  }
}
checkDestDir();
...
function getListMap() {
  ...
  const zipList = rawList.filter((file) => file.endsWith("zip"));
...
async function runAdd() {
  ...
  let anwser = await readlinePromise(
    `du you want to pack these ${fileList.length} files?`
  );
